...
public class UserTasksListController implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		// I <3 Java 8
		updateTasks = (Supplier<List<TaskSummary>>) Navigation.get().data().get(Param.UPDATE_USER_TASKS_ACTION);
		service = KieServerClientManager.getInstance();
		userId = (String) Navigation.get().data().get(Param.USER);
	...
	public void back() {
		Screen previous = (Screen) Navigation.get().data().get(Param.CALLER_SCREEN);
		Navigation.get().goTo(previous);
	}

	public void details() {
		Navigation.get().data().put(Param.DETAILS, tblTaskSummary.getItems());
